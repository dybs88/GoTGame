...
namespace GotGame.RestServer.Controllers
{
  ...
  public class GameListController : Controller
  {
    ...
    public GameListController(IGameListRepository gamesRepo, IPlayersRepository playersRepo, IChatRepository chatRepo, IGoTStorage storage)
    {
    ...
    public async Task<IActionResult> RefreshGame(int gameId, int playerId)
    {
      ...
      SessionItem newGameCreatorItem = goTStorage.GetItem(gameId, SessionKeys.NewGameCreator);
      if (newGameCreatorItem != null && !newGameCreatorItem.IsReadedByPlayerId(playerId))
      {
        ...
        if(newGameCreator != null)
        {
          ...
          goTStorage.TryRemoveItem(gameId, SessionKeys.NewGameCreator);
        }
      }
